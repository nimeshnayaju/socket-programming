...
while True:
  ...
  for key, mask in events:
    if key.data is None: # Accept the connection because its from a listening socket
      ...
      service_connection(key.mask) # key.mask is an event mask of the operations that are ready
...
def accept_wrapper(sock): # A function to get the new object and register it with selector
  ...
  data = types.SimpleNamespace(addr=addr, inb=b"", outb=b"") # Holds the socket data
